...
pub fn shield_absorb_damage(
    ...
    mut attacks_query: Query<(&mut Damage, &Target, &SourceTransform, &mut EffectLocation)>,
    mut shields_query: Query<(&mut Shield, &GlobalTransform)>,
) {
    for (mut damage, target, source_t, mut hit_loc) in attacks_query.iter_mut() {
            ...
            shields_query.get_mut(target.0.expect("target is none"))
        {
            ...
            commands.spawn().insert(CreateAnimatedEffect {
                transform: Transform::from_translation(shield_transform.translation)
                    * Transform::from_rotation(Quat::from_rotation_z(delta.y.atan2(delta.x)- std::f32::consts::FRAC_PI_2))
                    * Transform::from_scale(Vec3::splat(shield.radius / 64.0)),
                parent: None,
